...
namespace AssetsManagment.Controllers.distribute
{
    public class DistributeController : Controller
    {
        ...
        [HttpPost]
        public JsonResult showGridView()
        {

            SqlDataReader drGetGroup;
            List<IssueMester> oogrp = new List<IssueMester>();
            IssueResponse sp = new IssueResponse();
            Response response = new Response();

            string connectionString = WebConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString;
            strSQL = "SELECT *  ";
            strSQL = strSQL + "FROM ISSUE_VIEW ORDER BY CREATE_DATE ASC ";
            using (SqlConnection gcnMain = new SqlConnection(connectionString))
            {
                if (gcnMain.State == ConnectionState.Open)
                {
            ...
            //    while (drGetGroup.Read())
            //    {
            //        IssueMester ogrp = new IssueMester();
            //        ogrp.issue_no    = Convert.ToInt32(drGetGroup["ISSUE_NO"].ToString());

            //        ogrp.card_no = drGetGroup["CARD_NO"].ToString();
            //        ogrp.emp_name = drGetGroup["EMP_NAME"].ToString();
            //        ogrp.issue_date = Convert.ToDateTime(drGetGroup["ISSUE_DATE"]).ToString("dd-MM-yyyy");
            //         //ogrp.issue_date =Convert.ToDateTime( drGetGroup["ISSUE_DATE"]);
            //        //ogrp.issue_date =Utility.cvtSQLDate drGetGroup["ISSUE_DATE"])
            //        ogrp.department_name = drGetGroup["DEPARTMENT_NAME"].ToString();
        ...
        public JsonResult saveIssue(IssueMester resObje)
        {
            ...
            using (SqlConnection gcnMain = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    strSQL = strSQL + "(DEPARTMENT_CODE ,CARD_NO,EMP_NAME,ISSUE_DATE,";
                    ...
                    if (objIssue.gIssueDetailsList.Count > 0)
                    {
                        foreach (var item in objIssue.gIssueDetailsList)
                        {
                            ...
                            strSQL = strSQL + "'" + objIssue.created_by + "', " + Utility.cvtSQLDateString(stydate) + "";
                            strSQL = strSQL + ") ";
